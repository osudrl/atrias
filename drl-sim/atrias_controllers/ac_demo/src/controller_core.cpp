...
void controllerUpdate(robot_state state, ByteArray input, ControllerOutput *output, ByteArray &status)
{
    ...
    else if (currentState == DEMO_STATE_RUNNING)
    {
        if (currentDemo == DEMO1)
            desiredPos = demo1(&state, id, output, eclapsedTime);
        if (currentDemo == DEMO2)
            desiredPos = demo2(&state, id, output, eclapsedTime);
        if (currentDemo == DEMO3)
            desiredPos = demo3(&state, id, output, eclapsedTime);
...
void start_demo(robot_state* state, InputData* id, ControllerOutput* output)
{
    ...
    desiredPos.leg_ang = ((demoStart.leg_ang-lastDemoPos.leg_ang)/START_TIME) * eclapsedTime + lastDemoPos.leg_ang;
    desiredPos.leg_len = ((demoStart.leg_len-lastDemoPos.leg_len)/START_TIME) * eclapsedTime + lastDemoPos.leg_len;
    desiredPos.hip_ang = ((demoStart.hip_ang-lastDemoPos.hip_ang)/START_TIME) * eclapsedTime + lastDemoPos.hip_ang;
...
{
    // calculate current position
    desiredPos.leg_ang = (((PI/2) - lastDemoPos.leg_ang)/START_TIME) * eclapsedTime + lastDemoPos.leg_ang;
    desiredPos.leg_len = ((0.9 - lastDemoPos.leg_len)/START_TIME) * eclapsedTime + lastDemoPos.leg_len;
    desiredPos.hip_ang = ((0.135 - lastDemoPos.hip_ang)/START_TIME) * eclapsedTime + lastDemoPos.hip_ang;
    desiredPos.leg_ang_vel = 0.0;
    desiredPos.leg_len_vel = 0.0;
    desiredPos.hip_ang_vel = 0.0;
...
RobotPosition demo4(robot_state* state, InputData* id, ControllerOutput* output, float time)
{
    ...
    else
    {
        ...
        desTorqueA = (FORCE_P_GAIN * (state->desired_motor_angleA - state->motor_angleA)) - (FORCE_D_GAIN * (state->motor_velocityA));

        // Leg B, PD control.
        state->desired_motor_angleB = state->motor_angleB + (state->desired_spring_defB - spring_defB);
        desTorqueB = (FORCE_P_GAIN * (state->desired_motor_angleB - state->motor_angleB)) - (FORCE_D_GAIN * (state->motor_velocityB));
    }
