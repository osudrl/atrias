...
void controllerUpdate(robot_state state, ByteArray input, ControllerOutput *output, ByteArray &status)
{
	...
	else if (currentState == DEMO_STATE_RUNNING)
	{
		...
		if (currentDemo == DEMO4)
			desiredPos = demo4(state, id, output, 1.);
...
RobotPosition demo3(robot_state& state, InputData* id, ControllerOutput* output, double time)
{
	...
	// COMPUTE LENGTHS OF SWEEP PHASES
	float total_rel_phase = phase1 + phase2 + phase3 + phase4 + phase5 + phase6;

	float stop1 = phase1*period/total_rel_phase;
	float stop2 = phase2*period/total_rel_phase + stop1;
	float stop3 = phase3*period/total_rel_phase + stop2;
	float stop4 = phase4*period/total_rel_phase + stop3;
	float stop5 = phase5*period/total_rel_phase + stop4;
	float stop6 = phase6*period/total_rel_phase + stop5;
	...
	if(t <= stop1)
	{
		// Phase 1: Iterate tL, constant length

		// Compute tL limits
		float tLi = tA_min + PI - acos(l_max);
		float tLf = tB_max - PI + acos(l_max);

		// Compute time relative to ends of phase (0.0 - 1.0)
		float t_relative = (t - 0)/(stop1 - 0);
	...
	else if(t <= stop2)
	{
		...
		// Compute time relative to ends of phase (0.0 - 1.0)
		float t_relative = (t - stop1)/(stop2 - stop1);
	...
	{
		// Phase 3: Iterate tB, constant tA
		
		// Compute tB limits
		float tBi = tB_max;
		float tBf = 2*PI - 2*acos(l_min) + tA_max;

		// Compute time relative to ends of phase (0.0 - 1.0)
		float t_relative = (t - stop2)/(stop3 - stop2);
	...
	else if(t <= stop5)
	{
		...
		// Compute time relative to ends of phase (0.0 - 1.0)
		float t_relative = (t - stop4)/(stop5 - stop4);
	...
	{
		// Phase 6: Iterate tB, constant tA
		
		// Compute tB limits
		float tBi = tB_min;
		float tBf = 2*PI - 2*acos(l_max) + tA_min;

		// Compute time relative to ends of phase (0.0 - 1.0)
		float t_relative = (t - stop5)/(stop6 - stop5);
...
RobotPosition demo4(robot_state& state, InputData* id, ControllerOutput* output, double time)
{
	...
	else
	{
		...
		float desired_spring_defA = desired_spring_def;
		float desired_spring_defB = -1.0*desired_spring_def;

		// Leg A, PD control.
		float desired_motor_angleA = state.motor_angleA + (desired_spring_defA - spring_defA);
		desTorqueA = (FORCE_CONTROLLER_P_GAIN * (desired_motor_angleA - state.motor_angleA)) - (FORCE_CONTROLLER_D_GAIN * (state.motor_velocityA));

		// Leg B, PD control.
		float desired_motor_angleB = state.motor_angleB + (desired_spring_defB - spring_defB);
		desTorqueB = (FORCE_CONTROLLER_P_GAIN * (desired_motor_angleB - state.motor_angleB)) - (FORCE_CONTROLLER_D_GAIN * (state.motor_velocityB));
	}
