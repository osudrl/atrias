...
#include <ac_touchdown_velocity_test/controller_gui.h>
...
ControllerInitResult guiInit(Glib::RefPtr<Gtk::Builder> gui) {
    ...
    gui->get_widget("AccelP", accel_p_gain);
    gui->get_widget("AccelD", accel_d_gain);
    gui->get_widget("LegLength", desired_leg_length);
    gui->get_widget("AccelTime", acceleration_time);
    gui->get_widget("TD_Angle", touchdown_angle);
    ...
    if (stance_p_gain && stance_d_gain
            ...
            && accel_p_gain && accel_d_gain && desired_leg_length
            && acceleration_time && touchdown_angle
        ...
        accel_p_gain->set_range(0, 10000);
        accel_d_gain->set_range(0, 200);
        desired_leg_length->set_range(.3, 1.3);
        acceleration_time->set_range(0, 1.0);
        touchdown_angle->set_range(.5, 2.5);
        ...
        accel_p_gain->set_value(1000);
        accel_d_gain->set_value(40);
        desired_leg_length->set_value(0.9);
        acceleration_time->set_value(0.1);
        touchdown_angle->set_value(1.4);
...
void guiUpdate(robot_state state, ByteArray controllerStatus, ByteArray &controllerInput) {
    ...
    out.accelP           = accel_p_gain->get_value();
    out.accelD           = accel_d_gain->get_value();
    
    out.desiredLegLength = desired_leg_length->get_value();
    out.desiredTDAngle   = touchdown_angle->get_value();
    out.takeoff_height   = takeoff_height->get_value();
    out.touchdown_height = touchdown_height->get_value();
    out.desiredAccelTime = acceleration_time->get_value();
