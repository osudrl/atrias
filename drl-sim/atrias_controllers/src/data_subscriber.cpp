...
void datalogCallback(const atrias_controllers::AtriasData &aData) {
    ...
    if (isLogging) {
        if (log_file_fp == NULL) ROS_ERROR("data_subscriber cannot open log file.");
        else {
...
void guiCallback(const atrias_msgs::GUIInfo &guiInfo) {
    if (guiInfo.isLogging) {
        sprintf(buffer, guiInfo.logfileName.c_str());
        ROS_INFO("data_subscriber got logfile name: %s", buffer);
...
int main (int argc, char **argv) {
    ...
    ros::Subscriber gui_subscriber = nh.subscribe("gui_info", 1000, guiCallback);
