	...
	ControllerData *data)
{
	...
	else if (DEMO_CONTROLLER_STATE(state)->currentState = DEMO_STATE_RUNNING)
	{
		...
		if (DEMO_CONTROLLER_DATA(data)->commandedState = 0)
			DEMO_CONTROLLER_STATE(state)->DEMO_STATE_STOPPING;
	}
...
void startDemo(ControllerInput *input, ControllerOutput *output, ControllerState *state, ControllerData *data)
{
}

void stopDemo(ControllerInput *input, ControllerOutput *output, ControllerState *state, ControllerData *data)
{
}
