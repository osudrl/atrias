...
int gazebo_shmid;
SimulationEcatInterface *gazebo_shm;
...
//	// Can call this function if you have DC
//	Result = initEcatSlaveDc();
//	if (ECAT_FAILED(Result)) {
//		PRINT_MSG(MODULE_NAME": MainAppThreadFun: initEcatSlaveDc returned error: 0x%X(%s)\n",
//					Result, getEcatSlaveResultCodeDescription(Result));
//		goto out_releaseEcatSlave;
//	}

//	// Can call this function if you have DC latch
//	// Initialize slave's DC Latch inputs.
//	Result = initEcatSlaveDcLatch(TEST_DC_LATCH_ID, ECAT_FALSE, ECAT_FALSE);
//	if (ECAT_FAILED(Result)) {
//		PRINT_MSG(MODULE_NAME": MainAppThreadFun: initEcatSlaveDcLatch returned error: 0x%X(%s)\n",
//					Result, getEcatSlaveResultCodeDescription(Result));
//		goto out_releaseEcatSlave;
//	}
	...
	while (!g_IsExitRequested) 
	{
		...
		/*if (gazebo_shm->fresh)
		{
			...
			while (gazebo_shm->lock)
			{
				//PRINT_MSG("No shared memory access...\n");
				usleep(1);
			}
			gazebo_shm->lock = TRUE;*/
			updateEcatSlaveTxInputsMS(ECS_BOARD_ID, SM_INPUTS_1_ID, g_pInputsBuf);
			/*gazebo_shm->fresh = FALSE;
			gazebo_shm->lock = FALSE;

	#ifdef DEBUG_ECAT
			//PRINT_MSG("A:\tcommand = %u,\tstatus = %u\n",
			// gazebo_shm->atrias_input.ucontroller_inputA.command_byte, gazebo_shm->atrias_output.ucontroller_outputA.status_byte);
			//PRINT_MSG("mtA = %u,\tmtB = %u\n", 
			//	gazebo_shm->atrias_input.ucontroller_inputA.motor_torque, gazebo_shm->atrias_input.ucontroller_inputB.motor_torque);
			//PRINT_MSG("maA = %u,\tmaB = %u\n", gazebo_shm->atrias_output.ucontroller_outputA.transmission_position, 
			//	gazebo_shm->atrias_output.ucontroller_outputB.transmission_position);
			//PRINT_MSG("\n");

			//PRINT_MSG("%u\t%u\t%u\t%u\t%u\t%u\n", 
			//	gazebo_shm->atrias_input.ucontroller_inputA.command_byte, gazebo_shm->atrias_input.ucontroller_inputA.motor_torque,
			//	gazebo_shm->atrias_input.ucontroller_inputB.command_byte, gazebo_shm->atrias_input.ucontroller_inputB.motor_torque,
			//	gazebo_shm->atrias_input.ucontroller_input_hip.command_byte, gazebo_shm->atrias_input.ucontroller_input_hip.motor_torque );

			//PRINT_MSG("uc = %u,\tatrias = %u\n", sizeof(uControllerOutput), sizeof(AtriasOutput));

			//for (i = 0; i < g_InputsSize; i++)
			//{
			//	PRINT_MSG("%02X ", g_pInputsBuf[i]);
			//}

			gazebo_shm->atrias_output.ucontroller_outputA.status_byte = 1;
			gazebo_shm->atrias_output.ucontroller_outputA.timestamp = 1;
			gazebo_shm->atrias_output.ucontroller_outputA.spring_deflection = 1;
			gazebo_shm->atrias_output.ucontroller_outputA.rotor_position = 1;
			gazebo_shm->atrias_output.ucontroller_outputA.transmission_position = 1;
			gazebo_shm->atrias_output.ucontroller_outputA.thermistor1 = 1;
			gazebo_shm->atrias_output.ucontroller_outputA.thermistor2 = 1;
			gazebo_shm->atrias_output.ucontroller_outputA.thermistor3 = 1;

			gazebo_shm->atrias_output.ucontroller_outputB.status_byte = 2;
			gazebo_shm->atrias_output.ucontroller_outputB.timestamp = 2;
			gazebo_shm->atrias_output.ucontroller_outputB.spring_deflection = 2;
			gazebo_shm->atrias_output.ucontroller_outputB.rotor_position = 2;
			gazebo_shm->atrias_output.ucontroller_outputB.transmission_position = 2;
			gazebo_shm->atrias_output.ucontroller_outputB.thermistor1 = 2;
			gazebo_shm->atrias_output.ucontroller_outputB.thermistor2 = 2;
			gazebo_shm->atrias_output.ucontroller_outputB.thermistor3 = 2;

			gazebo_shm->atrias_output.ucontroller_output_hip.status_byte = 3;
			gazebo_shm->atrias_output.ucontroller_output_hip.timestamp = 3;
			gazebo_shm->atrias_output.ucontroller_output_hip.spring_deflection = 3;
			gazebo_shm->atrias_output.ucontroller_output_hip.rotor_position = 3;
			gazebo_shm->atrias_output.ucontroller_output_hip.transmission_position = 3;
			gazebo_shm->atrias_output.ucontroller_output_hip.thermistor1 = 3;
			gazebo_shm->atrias_output.ucontroller_output_hip.thermistor2 = 3;
			gazebo_shm->atrias_output.ucontroller_output_hip.thermistor3 = 3;

			PRINT_MSG("\n\n");
	#endif

		  //usleep(g_AppCyclePeriodMcs);
		}
		else
		{
			usleep(10);
...
int main(int argc, char** argv) {
	...
	/*==========================================================================*/
	// Share with Gazebo
	//gazebo_shm = (SimulationEcatInterface *)malloc(sizeof(SimulationEcatInterface));
	//gazebo_shm->lock = FALSE;

	// Create the segment.
	if ((gazebo_shmid = shmget(GAZEBO_SHM_KEY, SHMSZ, IPC_CREAT | 0666)) < 0) {
		  perror("shmget");
		  exit(1);
	}

	// Now we attach the segment to our data space.
	if ((gazebo_shm = (SimulationEcatInterface *)shmat(gazebo_shmid, NULL, 0)) == (char *) -1) {
		  perror("shmat");
		  exit(1);
	}
	...
	g_pOutputsBuf = (UINT8 *)&gazebo_shm->atrias_input;
	g_pInputsBuf = (UINT8 *)&gazebo_shm->atrias_output;
