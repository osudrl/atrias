...
#include "sensor_limit_gatherer_kern.h"
...
void run(long data)
{
	...
	last_tranB_cnt = DISCRETIZE(controller_input.leg_angleB,
	...
	tranB_off = (last_tranB_cnt / MAX_13BIT) * MAX_13BIT;
  ...
  while (true) 
	{
      ...
      rt_sem_signal(&master_sem);
			...
			if (domain1_state.working_counter == EC_WC_ZERO) //EC_WC_COMPLETE
			{
				...
				if (((uControllerOutput *)(domain1_pd + off_medulla_tx[medulla_B_index]))->TRANS_ANGLE - last_tranB_cnt > ROLLOVER13BIT_THRESHOLD)
				{
					tranB_off -= MAX_13BIT;
				}
				if (last_tranB_cnt - ((uControllerOutput *)(domain1_pd + off_medulla_tx[medulla_B_index]))->TRANS_ANGLE > ROLLOVER13BIT_THRESHOLD)
				{
					tranB_off += MAX_13BIT;
				}
			...
			else
			{
				...
				if (!uspace_shm->lock)
				{
					...
					uspace_shm->medullaB_biss_encoder		= ((uControllerOutput *)(domain1_pd + off_medulla_tx[medulla_B_index]))->LEG_SEG_ANGLE; 
					uspace_shm->medullaA_ssi_encoder 		= ((uControllerOutput *)(domain1_pd + off_medulla_tx[medulla_A_index]))->TRANS_ANGLE; 
					uspace_shm->medullaB_ssi_encoder		= ((uControllerOutput *)(domain1_pd + off_medulla_tx[medulla_B_index]))->TRANS_ANGLE;

					//uspace_shm->boom_pan_enc						= ((uControllerOutput *)(domain1_pd + off_medulla_tx[medulla_boom_index]))->BOOM_PAN_ANGLE;
					//uspace_shm->boom_tilt_enc						= ((uControllerOutput *)(domain1_pd + off_medulla_tx[medulla_boom_index]))->BOOM_TILT_ANGLE;
					//uspace_shm->boom_roll_enc						= ((uControllerOutput *)(domain1_pd + off_medulla_tx[medulla_boom_index]))->BOOM_ROLL_ANGLE;
			...
			//rt_printk("timestep = %u\n", ((uControllerOutput *)(domain1_pd + off_medulla_tx[0]))->timestep);

			// Put controller here.
			/*rt_printk("ID 1: %02X,\tID 2: %02X,\tID 3: %02X,\tID 4: %02X\n", ((uControllerOutput *)(domain1_pd + off_medulla_tx[0]))->id, 
				((uControllerOutput *)(domain1_pd + off_medulla_tx[1]))->id, ((uControllerOutput *)(domain1_pd + off_medulla_tx[2]))->id,
				((uControllerOutput *)(domain1_pd + off_medulla_tx[3]))->id);*/
