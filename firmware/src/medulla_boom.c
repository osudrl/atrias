...
void boom_initilize(uint8_t id, ecat_slave_t *ecat_slave, uint8_t *tx_sm_buffer, uint8_t *rx_sm_buffer, medulla_state_t **commanded_state, medulla_state_t **current_state, TC0_t *timestamp_timer, uint16_t **master_watchdog) {
	...
	#endif
	ecat_init_sync_managers(ecat_slave, rx_sm_buffer, 3, 0x1000, tx_sm_buffer, 23, 0x2000);

	#ifdef DEBUG_HIGH
	printf("[Medulla Boom] Initilizing PDO entries\n");
	#endif
	ecat_configure_pdo_entries(ecat_slave, boom_rx_pdos, 3, boom_tx_pdos, 15); 
