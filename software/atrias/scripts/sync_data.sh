...
if mount|grep "on $MOUNT_DIR type"; then
	# Already mounted
	printf "$WARNING[Already mounted!]$ENDC\n"
...
file_list=$(find "$SOURCE_DIR" -maxdepth 1 -name "*.bag" -type f)
total_count=$(find "$SOURCE_DIR" -maxdepth 1 -name "*.bag" -type f|wc -l)
...
# Exit message
printf "$OKBLUE[Exiting!]$ENDC sync_data.sh\n"
exit 0
