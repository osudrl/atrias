...
SORT_DIR="${SOURCE_DIR}/sorted"
#SYNC_DIR="/mnt/attic/experimental-data/atrias-2-1"
SYNC_DIR="/media/drl-covell-021/experimental-data/atrias-2-1"
...
if [ -z "$(ls ${SOURCE_DIR})" ]; then
	printf "${FAIL}[Error!]${ENDC} Nothing to sync!\n"
	exit 0
fi
...
# To hurst attic network drive
#rsync --recursive --checksum --verbose --compress --human-readable --progress "${SORT_DIR}/" "${SYNC_DIR}/"
# To external HDD connected to gui laptop
rsync --recursive --checksum --verbose --compress --human-readable --progress "${SORT_DIR}/" -e ssh drl@${GUI_HOSTNAME}:"${SYNC_DIR}/"
printf "${OKGREEN}[Done!]${ENDC}\n\n"
