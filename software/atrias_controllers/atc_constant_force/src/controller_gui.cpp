 ...
 * controller_gui.cpp
 *
 * atc_constant_force controller
 *
 *  Created on: May 5, 2012
 *      Author: Michael Anderson
 */

#include <atc_constant_force/controller_gui.h>
...
bool guiInit(Glib::RefPtr<Gtk::Builder> gui) {
    ...
    if (torque_A_hscale && torque_B_hscale && torque_hip_hscale) {
        ...
        sub = nh.subscribe("atc_constant_force_status", 0, controllerCallback);
        pub = nh.advertise<atc_constant_force::controller_input>("atc_constant_force_input", 0);
        return true;
    }
    return false;
}
...
void controllerCallback(const atc_constant_force::controller_status &status) {
