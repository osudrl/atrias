...
std::tuple<double, double, double, double> ATCForceControlDemo::automatedPositionTest(double t) {
	...
	if (t >= 0*duration && t < 1*duration) {
		...
		std::tie(qmA, qmB) = ascCommonToolkit.legPos2MotorPos(PI/2.0, 0.75);
		...
		b = 2.0*PI*omega1;
		ql = PI/2.0;
		rl = 0.8 + 0.1*sin(a*pow(tOffset, 2) + b*tOffset);
		dql = 0.0;
		drl = 0.1*cos(a*pow(tOffset, 2) + b*tOffset)*(b + 2.0*a*tOffset);		
