 ...
 * controller_gui.cpp
 *
 * atc_mikhail_test controller
 *
 *  Created on: May 5, 2012
 *      Author: Michael Anderson
 */

#include <atc_mikhail_test/controller_gui.h>
...
bool guiInit(Glib::RefPtr<Gtk::Builder> gui) {
    ...
    if (torque_A_hscale && torque_B_hscale && torque_hip_hscale) {
        ...
        sub = nh.subscribe("atc_mikhail_test_status", 0, controllerCallback);
        pub = nh.advertise<atc_mikhail_test::controller_input>("atc_mikhail_test_input", 0);
        return true;
    }
    return false;
}
...
void controllerCallback(const atc_mikhail_test::controller_status &status) {
