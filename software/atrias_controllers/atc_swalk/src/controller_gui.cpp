 ...
 * controller_gui.cpp
 *
 * atc_swalk controller
 *
 *  Created on: May 5, 2012
 *	  Author: Michael Anderson
 */

#include <atc_swalk/controller_gui.h>
...
bool guiInit(Glib::RefPtr<Gtk::Builder> gui) {
	...
	if (control_combobox && lhip_pos_spinbutton && rhip_pos_spinbutton && gc_l_button && gc_r_button &&
		...
		d_as_spinbutton->set_range(0, 2.0);
		...
		sub = nh.subscribe("ATCSWalk_status", 0, controllerCallback);
		pub = nh.advertise<atc_swalk::controller_input>("ATCSWalk_input", 0);
		return true;
	}
	return false;
}
